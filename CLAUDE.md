...
한 판을 끝까지 플레이할 수 있고, 끊겨도 30초 안에 돌아오면 자리가 복구되며,
**늦게 온 사람은 관전하다가 다음 라운드에 자동 참여한다.**
연속 라운드·점수·연출은 아직 없다 (M4~M5).
...
npm test           # 111개 (정보 은닉 바이트 테스트 포함)
...
- 페이즈 `scoreboard`·`match-result`가 이미 전이 테이블에 있다 (accepts까지).
- 점수 상수도 `shared/constants.ts`에 있다 — 라이어 승 +2 / 시민 승 +1 /
  정확 지목 +1. **플레이 후 조정할 수 있게 상수로 둔 것이다.**
- `round-result`에서 `backToLobby()` 대신 `scoreboard`로 가고,
  `totalRounds`(3/5/무제한)에 도달하면 `match-result`로 간다.
- 정확 지목 보너스는 그 라운드의 **마지막 지목** 기준이다.
- 무효 라운드는 점수 변동이 없고 **라운드 수를 소모하지 않는다.**
- 라이어 선정은 매 라운드 **완전 독립 시행**이다 (D10). 로테이션 금지.
