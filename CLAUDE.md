...
**게임의 기능은 사실상 완성됐다.** 연속 라운드·누적 점수·순위까지 동작한다.
남은 것은 연출(M5)과 운영(M6)이다.
...
연출은 부가가 아니라 기능이다 (REQUIREMENTS §F9). 자리는 이미 잡혀 있다.

- `shared/constants.ts`의 `DESCRIPTION_REVEAL_MS`(→3500)와
  `VOTE_REVEAL_MS`(→12000)를 **상수만 바꾸면** 연출 페이즈가 살아난다.
  상태 머신을 다시 짜지 않는다 — M1에서 자리를 비워둔 이유가 이것이다.
- `client/app.css`에 v1 키프레임 14종이 이미 있다.
- 서버는 페이즈와 남은 시간만 준다. **타임라인은 클라이언트가 소유한다.**
  Svelte의 `transition:`/`animate:`/`spring`으로 표현하고,
  v1이 손으로 짠 setInterval 체인을 대체한다.
- 이식할 연출: 순서 추첨, 한줄 설명 타이핑, 개표 카운트다운,
  라이어 공개("○○님은 라이어가…" → "맞습니다!/아닙니다!"), 지목 펄스.
