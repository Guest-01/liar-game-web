...
게임 기능과 연출이 모두 들어갔다. 남은 것은 운영(M6)이다 —
피드백·SEO·배포 파이프라인, 그리고 **브라우저 스모크 테스트**.
...
npm test           # 147개 (정보 은닉 바이트 테스트 포함)
...
- **브라우저 스모크 테스트를 CI에 넣는다.** 지금까지 클라이언트에서 검증된 것은
  타입과 빌드뿐이다. `rune_outside_svelte`처럼 정적 검사가 하나도 못 잡고
  브라우저에서만 죽는 부류가 실제로 나왔다 (체크리스트 G12).
  로비 로드 → 방 생성 → 참가까지 실제로 클릭하는 테스트가 필요하다.
- 인앱 피드백 (Discord 웹훅, 부팅 시 유효성 검증 후 기능 토글)
- OG 이미지, sitemap 갱신
- `docs/V1-TEST-CASES.md` 130건을 이식/변경/폐기로 분류 마무리
