...
`v2` 브랜치는 **한 판을 끝까지 플레이할 수 있는 상태**다.
방 생성 → 참가 → 대기실 → 1라운드 완주가 동작한다.
관전·재접속·점수·연출은 아직 없다 (M2~M5).
...
`allowReconnection(client, 30)` + `PhaseTimer`의 일시정지를 연결한다.
골격은 이미 있다 — `server/rooms/timer.ts`의 `onDisconnect/onReconnect`,
`RoomSchema.isPaused/pausedFor`, 클라이언트의 일시정지 배너까지 자리를 잡아뒀다.

- `LiarRoom.onLeave`가 지금은 즉시 이탈 처리한다. 여기에 유예를 넣는다.
- 라이어 이탈 단계별 차등(R3)도 M2에서 유예와 함께 넣는다.
- `client/lib/session.ts`의 `saveReconnectToken/loadReconnectToken`이 이미 준비되어 있다.
