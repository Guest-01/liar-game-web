<script lang="ts">
  import { game, send } from "../lib/connection.svelte.js";
  ...
  const canSpeak = $derived(!isDefense || game.snapshot?.defendantId === game.mySessionId);
...
<div class="bg-gray-800 rounded-xl flex flex-col h-full min-h-0">
  ...
  <form onsubmit={submit} class="p-3 border-t border-gray-700 flex gap-2">
    <input bind:value={text} maxlength={CHAT_MAX} disabled={!canSpeak}
           placeholder={canSpeak ? "메시지 입력…" : "최후 변론 중에는 피고만 발언할 수 있습니다"}
