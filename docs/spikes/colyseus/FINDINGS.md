...
## 9-A. 추가 검증 — 상태 어댑터 (프레임워크 선택 해제)

`state.toJSON()`이 **StateView를 그대로 존중하는 평범한 JS 객체**를 만든다.
타인의 비밀 필드는 빈 값이 아니라 **키 자체가 존재하지 않는다.**

```
bob의 스냅샷: [{"alice":null}, {"bob":"SENTINEL_CITIZEN_김치찌개"}, {"carol":null}]
                  ↑ myWord 키 없음                                     ↑ 키 없음
```
...
| `toJSON()` 평균 | **0.0092ms** |
| p95 | 0.0289ms |
| 60fps 프레임 예산 대비 | **0.055%** |

→ 어댑터가 한 줄이면 끝난다:

```ts
room.onStateChange(state => { store.snapshot = state.toJSON() })
```

**결론**: 세밀한 Schema 콜백이 필요 없다. 따라서 **Colyseus는 클라이언트 프레임워크
선택에 아무 제약을 걸지 않는다.** 선택은 연출(F9) 편의성과 번들 크기로만 결정하면 된다.

## 9-B. 추가 검증 — 일시정지 (R2)

`Delayed`의 `pause()` / `resume()` / `elapsedTime`으로 R2가 성립한다. 7/7 통과.

```
pause() 직후 elapsedTime 기록      291ms
정지 중 발화하지 않음               ✅
정지 중 elapsedTime 멈춤            291ms → 291ms
남은 시간 = 1000 - 291 = 709ms      ← 클라이언트에 보낼 값 (절대시각 아님)
resume() 후 실제 경과                693ms (기대 ~709ms)
```

> ### ⚠️ 함정: `pause()`는 중첩 카운트를 하지 않는다
> `pause()`를 두 번 호출해도 **`resume()` 한 번이면 재개된다.**
> 2명이 끊겼다가 1명만 복귀했을 때 타이머가 살아나 버린다.
> **끊긴 인원 수를 직접 세야 한다.** → 체크리스트 G8.

---

## 10. 클라이언트 스택 결정 (2026-08-18)

9-A로 프레임워크 제약이 사라졌으므로, 요구사항만으로 선택했다.

서버 권위 설계에서 클라이언트의 상태 관리 부담은 거의 사라진다.
남는 일의 무게중심은 **F9 연출**이다 (v1 `game.js` 983줄 중 ~300줄이 손으로 짠 애니메이션).

| | 런타임(gzip) | F9 연출 |
|---|---|---|
| **Svelte 5** ← 채택 | ~10KB | `transition:` / `animate:` / `spring` 내장 |
| Vue 3 | ~34KB | `<Transition>` |
| React | ~45KB + 애니메이션 라이브러리 | 별도 라이브러리 필요 |
| Alpine (v1) | ~15KB | 직접 구현 |

**채택: Svelte 5 + Vite.** SDK가 이미 39.7KB를 쓰므로 가장 가벼운 런타임이
그 비용을 상쇄하고, 무게중심인 연출을 선언적으로 해결한다.

**페이지 구성**: 전 경로 단일 SPA + 서버가 경로별 OG 메타를 주입한 HTML 셸.
`/room/:id`는 색인하지 않되 **카카오톡 링크 공유 미리보기 때문에 OG는 필요**하다.
풀 SSR 메타프레임워크는 쓰지 않는다.

→ 상세는 `docs/ARCHITECTURE.md`.
